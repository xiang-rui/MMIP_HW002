...
# roi_map_bits(u32) roi_map_bytes(u32)
# nstages(u8) reserved(3)
HDR_FMT = "<4sBBBBHHHHHHII B3s"
...
def write_header(f, *, flags, bitdepth, blockN, width, height, padW, padH,
                 qstep_bg, qstep_roi, roi_map_bits, roi_map_bytes, nstages):
    f.write(struct.pack(
        HDR_FMT, MAGIC, VERSION, flags, bitdepth, blockN,
        width, height, padW, padH, qstep_bg, qstep_roi,
        roi_map_bits, roi_map_bytes, nstages, b"\x00\x00\x00"
    ))
...
def read_header(f):
    ...
    (magic, ver, flags, bitdepth, blockN,
     width, height, padW, padH, qbg, qroi,
     roi_bits, roi_bytes, nstages, _) = struct.unpack(HDR_FMT, data)
    if magic != MAGIC:
        raise ValueError("Bad magic")
    if ver != VERSION:
        raise ValueError(f"Unsupported version: {ver}")
    return dict(flags=flags, bitdepth=bitdepth, blockN=blockN,
                width=width, height=height, padW=padW, padH=padH,
                qstep_bg=qbg, qstep_roi=qroi,
                roi_map_bits=roi_bits, roi_map_bytes=roi_bytes,
                nstages=nstages)
